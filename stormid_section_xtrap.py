...
def storm_objects_new(refc,proj,REFlev,REFlev1,big_storm,numsecs,smoothed_ref,ax,rlons,rlats,storm_index,tracking_index,scan_index,tracks_dataframe,track_dis,time_start):
    g = Geod(ellps='sphere')
    #Inputs,
    ...
    #numsecs: Min number of seconds over which to calculate storm motion
    ...
    #track_dis: Maximum distance for dislocation of storm objects between iterations to be considered same object
    #time_start: Time of the current volume scan
                            ...
                            for contour_poly1 in level1.get_paths(): 
                                ...
                                if pr_area1 > 10 * units('km^2') and meanr1 > REFlev1[0]:
                                    ...
                                    if tracking_index == 0:
                                        ...
                                        ax.scatter(prj_lon, prj_lat, s=200, color='orange')
    ...
    for i_d in unique_ids:
        ...
        if (ids_i.shape[0]) > 1:
            ...
            for k_d in range(ids_i.shape[0]):
                ...
                #Add a try statement for the extremely rare circumstance that a duplicate slips through for one scan.
    ...
    for c in range(len(storm_ids)):
        print()
        print("calculating storm motion vectors for all cells...")
        print()
        ...
        try:
            ...
            ### New code by MJB on 12/15/2020 to use linear least squares regression for given number of seconds
            #secs_ago = (time_start-times_str[0]).seconds
            #print("secs_ago =", secs_ago)
            print("len_str =", len_str)
            stopsearch = 0
            ##
            ## SEARCH back in time based on "numsecs" specified by the user
            ##
            for radscans in range(len_str-1, -1, -1):
                secs_ago = (time_start-times_str[radscans]).seconds
                print("secs_ago from current time =", secs_ago, "; radscan ago =", len_str-radscans)
                if secs_ago > numsecs and stopsearch == 0:
                    print(secs_ago, "sec", "numscans =", len_str-radscans)
                    numscans = len_str-radscans
                    stopsearch = 1
                else:
                    matt = 51
            
            ind_tr_start = len_str - numscans
            if ind_tr_start < 0:
                ind_tr_start = 0
            track_token = 0
            ...
            Y.append(max_lat_te)
            X_obs2 = max_lon_te
            for radscans in range(len_str-1, ind_tr_start-1, -1):
                print(radscans)
                X.append(max_lon_str[radscans])
                Y.append(max_lat_str[radscans])
                X_obs1 = max_lon_str[radscans]
            print("X = ", X)
            print("Y = ", Y)
            Xlon = np.array(X)
            Ylat = np.array(Y)
            ...
            print()
            print("ind_tr_start =", ind_tr_start, "len_str =", len_str)
        except:
            print('looks like a new cell...or we do not have enough scans')
            print()
            track_token = 1
        ...
        if track_token == 0:
            print()
            print("start_time =", times_str[ind_tr_start], ", end_time =", time_start, ", sloni =", max_lon_str[ind_tr_start], ", slati =", max_lat_str[ind_tr_start], ", slone =", max_lon_te, ", slate =", max_lat_te)
            print()
            ...
            print()
            #Now using linear least squares regression instead of just the first and last centroid position
            st_speed, st_direction = radar_motion(times_str[ind_tr_start], time_start, X_obs1, Y_pred1, X_obs2, Y_pred2)
